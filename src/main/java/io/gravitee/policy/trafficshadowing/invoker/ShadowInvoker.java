...
public class ShadowInvoker implements Invoker {
    ...
    private void invoke(
        ...
        final HttpHeaders shadowHeaders = addShadowingHeaders(context.request().headers(), context);
    ...
    private HttpHeaders addShadowingHeaders(HttpHeaders headers, ExecutionContext context) {
        ...
        if (configuration.getHeaders() != null) {
            configuration.getHeaders().forEach(header -> addShadowingHeader(context, shadowHeaders, header));
        }
        return shadowHeaders;
    }

    private void addShadowingHeader(ExecutionContext context, HttpHeaders headers, HttpHeader header) {
        try {
            ...
            LOGGER.debug("Shadowing header raised an error and won't be added to the request", e);
        }
    }
}
